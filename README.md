...
All datasets are available in [Hugging Face](https://huggingface.co/datasets/xinyulu/vibench). 
...
We ensure the reproducibility of our datasets by providing documented workflows in the `generate_pah_dataset.ipynb` and `generate_tripeptides_dataset.ipynb` notebooks. All molecules within the VB-Mols dataset can be retrieved using their respective ZINC-ID or GDB-ID, which are stored under the `filename` field in the LMDB files.
...
@article{lu2025vib2mol,
      ...
      author={Xinyu Lu, Hao Ma, Hui Li, Jia Li, Yi Rong, Yuqiang Li, Tong Zhu, Guokun Liu, Bin Ren},
      year={2025},
