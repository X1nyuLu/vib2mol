...
class Engine(BaseEngine):
    ...
    def eval_epoch(self, epoch):
        ...
        return {'loss':eval_losses.avg, 'clip_loss':eval_losses_clip.avg, 'lm_loss':eval_losses_lm.avg, 'recall':spectrum_to_smiles_recall, 'acc':eval_acc.avg}
    ...
    def train(self):
        for epoch in range(self.config['epoch']):
            ...
            if self.engine.device_rank == 0:
                ...
                self.writer.add_scalar('eval_accuracy', eval_output['acc'], epoch)

                if 'acc' in eval_output:
                    self.es(eval_output['acc'], self.model,
                    f"{self.model_save_path}/epoch{epoch}_acc{eval_output['acc']*100:.0f}.pth")
                else:
...
@register_function('cl_lm')
def train_cl_lm_model(*args, **kwargs):
